...
using static SpeechEngine.Speech.VoskModelInfos;
...
public class SpeechRecognizer(
    ...
    AudioRecorder _audioRecorderService,
    WebRequestsService _webRequestsService,
    ISpeechEngineParametersService _speechEngineParametersService)
    : IDisposable
{
    ...
    public async Task RecognizeFromDiskAsync(Action<string, bool> updateResult, string filePath)
    {
        if (_isRecognizingFromDisk)
            await _speechEngineParametersService.SendTryLaterAsync();
    ...
    private async Task<bool> CheckModelFolderAsync(VoskModelInfo voskModelInfo)
    {
        ...
        {
            if (!await _speechEngineParametersService.CheckBeforeDownloadingSpeechModelAsync())
                return false;
            
            await _webRequestsService.DownloadAsync(
                new Uri(voskModelInfo.Uri),
                voskModelInfo.ArchivePath,
                (percentages) => _speechEngineParametersService.UpdateDownloadingBar(new SpeechModelDownloadingProgressBar() { Percentages = percentages }));
        }
